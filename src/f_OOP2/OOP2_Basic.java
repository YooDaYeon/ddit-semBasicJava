...
public class OOP2_Basic {
	public static void main(String[] args) {
		...
		4. 단일상속
		  ...
		  - TV와 VCR에 동일명의 method가 있다면 어떤 것을 상속받을지 알수 없다.
		...
		5. Object클래스(모든 클래스의 조상)
		  ...
		  - 다른클래스로부터 상속받지 않는 클래스는 자동으로 Object클래스를 상속 받는다.
		    class Tv{
		...
		6. 오버라이딩(override)
		  ...
		  - 조상클래스의 메서드를 자손클래스에서 오버라이딩 할때
		    접근제어자를 조상 클래스의 메서드보다 좁은 범위로 설정할 수 없다.
		    예외는 조상클래스의 메서드보다 많이 선언할 수 없다.
		  - 인스턴스 멤버를 클래스 멤버로 또는 그반대로 변경할 수 없다.
		
		7. super.
		  - 조상클래스의 멤버와 자손클래스의 멤버가 중복정의되어 있을때 서로 구별하기
		    위한 용도로 사용
		  - 조상멤버와 자신의 멤버를 구분하는데 사용한다는 점을 제외하고는 this와
		    근본적으로 같다.		
		
		8. super() - 자식클래스의 생성자에서 부모클래스의 생성자를 호출할때 사용
		...
		9. 제어자
		  ...
		  - static(클래스의, 공통적인)
		    ...
		    : 클래스변수
		      ...
		      - 클래스가 method Area로드될대 생성
		      
		  - final(마지막의, 변경할수 없는)
		    : 변수에 사용시 변경할수 없는 상수가 된다.
		    : 메서드에 사용시 오버라이딩 할 수 없다.
		    : 클래스에 사용시 다른클래스의 조상이 될 수 없다.
		...
		12. 생성자의 접근제어자
		  ...
		  - 생성자의 접근제어자를 private으로 지정
		    ...
		    : 다른클래스의 조상이 될수 없다.
		    : 상속을 할수 없는 클래스 임으로 final을 추가하여 상속할수 없는 클래스임을
		      알리는 것이 좋다.
		...
		17. 여러객체배열 Vector
		  ...
		  Object get(int index) : index번째 방의 객체를 반환한다.
		...
		19. 인터페이스(interface)
		  ...
		  - 인터페이스의 구현(추상 클래스와 비슷하다)
		    ...
		    : ~able(~을 할수 있는) => IMember -> IMemberImpl
		  ...
		  - 인터페이스의 이해
		    ...
		    : 메서드는 사용하는 쪽은 사용하려는 메서드의 선언부만 알면된다.
		    A - I - B
